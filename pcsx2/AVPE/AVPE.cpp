...
namespace AVPE
{
		...
		const NativeMenuInput::PointerResult& result)
	{
		char response[960];
		std::snprintf(response, sizeof(response),
			R"({"pointer":"0x%08X","callback":"0x%08X","handler":"0x%08X","callback_count":%u,"before":{"focus_handle":"0x%08X","focus_object":"0x%08X"},"after":{"focus_handle":"0x%08X","focus_object":"0x%08X"},"focused_item_action":"0x%08X","focused_item_action_valid":%s,"screen_x":%.6f,"screen_y":%.6f,"observed_x":%.6f,"observed_y":%.6f,"menu_x":%.6f,"menu_y":%.6f,"staging_address":"0x%08X","return_pc":"0x%08X","stopped_pc":"0x%08X","last_avpe_text_pc":"0x%08X","stack_restored":%s,"elapsed_cycles":%llu,"deferred":%s,"dispatch_pointer_id":%llu,"deferred_call_id":%llu})",
			result.pointer, result.callback, result.handler, result.callback_count, result.before.handle,
			result.before.object, result.after.handle, result.after.object, result.focused_item_action,
			result.focused_item_action_valid ? "true" : "false", result.screen_x,
			...
			static_cast<unsigned long long>(result.dispatch_pointer_id),
	...
	static lucent::http::Response dispatch(const lucent::http::Request& req)
	{
		...
		if (auto response = NativePromptRoute::Handle(req))
		{
			return std::move(*response);
		}
		...
		if (req.method == "GET" && path == "/snap")
			return NativeSnapshotRoute::Handle();
